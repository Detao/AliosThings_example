...
int mqtt_client(void)
{
    ...
    do {
        ...
        rc=get_temperature_data(&cnt);
        if(rc)
        {
            printf("get temperature filed ！\n");
            return ;
        }
        msg_len = snprintf(msg_pub, sizeof(msg_pub), "{\"id\":\"1\",\"version\":\"0.1\",\"params\":{\"temperature\":%.2f% },\"method\":\"thing.event.property.post\"}", cnt);
        if (msg_len < 0) {
