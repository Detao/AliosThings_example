...
extern aos_sem_t s_xSemaphore;
extern aos_mutex_t  s_xMutex;
...
void DM9000_Reset(void)
{
	...
	aos_msleep(100);								//一定要有这个延时，让DM9000准备就绪！
...
void DM9000_SendPacket(struct pbuf *p)
{
    ...
    aos_mutex_lock(&s_xMutex,AOS_WAIT_FOREVER);
...
void DMA9000_ISRHandler(void)
{
	...
	{  
 		aos_sem_signal(&s_xSemaphore);   //处理接收到的 数据包
	} 
