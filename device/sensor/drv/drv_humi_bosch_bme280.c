...
static int drv_humi_bosch_bme280_get_cali_temp(void)
{
    int     ret;
...
static int drv_humi_bosch_bme280_get_cali_humi(void)
{
    int     ret = 0;
    int16_t dig_H4_lsb;
    int16_t dig_H4_msb;
...
static int drv_humi_bosch_bme280_get_cali_parm(void)
{
    int     ret                                   = 0;

    ret = drv_humi_bosch_bme280_get_cali_temp();
    if (unlikely(ret)) {
        return -1;
    }

    ret = drv_humi_bosch_bme280_get_cali_humi();
    if (unlikely(ret)) {
        return -1;
    }
...
static int drv_humi_bosch_bme280_soft_reset(void)
{
    int     ret   = 0;
...
static int drv_humi_bosch_bme280_validate_id(uint8_t id_value)
{
    uint8_t value = 0x00;
    int     ret   = 0;
...
static int drv_humi_bosch_bme280_enter_sleep_mode(void)
{
    ...
    ret = bme280_io_read(BME280_CTRL_HUM_ADDR, data, 4);
    if (unlikely(ret)) {
        return ret;
    }
...
static int drv_humi_bosch_bme280_set_work_mode(uint8_t mode)
{
    uint8_t ret   = 0;
    uint8_t value = 0;
    uint8_t temp  = 0;
...
static int drv_humi_bosch_bme280_set_power_mode(dev_power_mode_e mode)
{
    uint8_t value = 0x00;
    int     ret   = 0;
...
static int drv_humi_bosch_bme280_set_odr(uint8_t odr)
{
    int     ret       = 0;
    uint8_t v_data_u8 = 0;
...
static int drv_humi_bosch_bme280_set_default_config(void)
{
    int ret = 0;

    ret = drv_humi_bosch_bme280_set_power_mode(DEV_SLEEP);
    if (unlikely(ret)) {
        return ret;
    }

    ret = drv_humi_bosch_bme280_set_odr(BME280_DEFAULT_ODR_1HZ);
    if (unlikely(ret)) {
        return ret;
    }
...
static int drv_humi_bosch_bme280_cali_temp(void)
{
    ...
    temp.t = (int32_t)((uint32_t)data[0] << BME280_SHIFT_BY_12_BITS) |
             ((uint32_t)data[1] << BME280_SHIFT_BY_04_BITS) |
             ((uint32_t)data[0] >> BME280_SHIFT_BY_04_BITS);
...
static int drv_humi_bosch_bme280_open(void)
{
    ...
    ret = drv_humi_bosch_bme280_set_work_mode(BME280_STANDARD_RESOLUTION_MODE);
    if (unlikely(ret)) {
        return -1;
    }

    ret = drv_humi_bosch_bme280_set_power_mode(DEV_POWER_ON);
    if (unlikely(ret)) {
        return -1;
    }
    return 0;
...
{
    int ret = 0;
    ret     = drv_humi_bosch_bme280_set_power_mode(DEV_POWER_OFF);
    if (unlikely(ret)) {
        return -1;
    }
    return 0;
...
static int drv_humi_bosch_bme280_ioctl(int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SENSOR_IOCTL_ODR_SET: {
            ...
            ret         = drv_humi_bosch_bme280_set_odr(odr);
            if (unlikely(ret)) {
                return -1;
            }
        } break;
        case SENSOR_IOCTL_SET_POWER: {
            ret = drv_humi_bosch_bme280_set_power_mode(arg);
            if (unlikely(ret)) {
                return -1;
            }
        } break;
